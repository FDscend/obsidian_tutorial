...
var PROOF_LIKE_ENV_IDs = ["proof", "solution"];
          ...
          editButton
            ...
            .onClick(() => {
              const copied = JSON.parse(
                JSON.stringify(this.plugin.extraSettings.profiles[id])
              );
...
var EditProfileModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
        ...
        text.setValue(this.profile.meta.tags.join(", ")).onChange((value) => {
          ...
          if (
            ...
            new import_obsidian3.Notice(
              "A tag can only contain lower-case alphabets or hyphens.",
              5e3
            );
          }
        });
    ...
    new import_obsidian3.Setting(contentEl)
      .setName("Theorem-like environments")
      .setHeading();
    for (const envID of THEOREM_LIKE_ENV_IDs) {
      this.settingRefs[envID] = new import_obsidian3.Setting(contentEl)
        .setName(envID)
        .addText((text) => {
          var _a;
          text
            .setValue((_a = this.profile.body.theorem[envID]) != null ? _a : "")
            .onChange((value) => {
    ...
    for (let i = 0; i < PROOF_SETTING_KEYS.length; i++) {
      ...
      this.settingRefs[key] = new import_obsidian3.Setting(contentEl)
        .setName(name)
        .addText((text) => {
          var _a;
          text
            .setValue((_a = this.profile.body.proof[key]) != null ? _a : "")
            .onChange((value) => {
              this.profile.body.proof[key] = value;
            });
        });
    }
    const linkedProofHeading = new import_obsidian3.Setting(contentEl)
      .setName("Linked proofs")
      .setDesc(
        `For example, you can render \`${
          DEFAULT_SETTINGS.beginProof
        }\`@[[link to Theorem 1]] as "${
          DEFAULT_PROFILES[DEFAULT_SETTINGS.profile].body.proof
            .linkedBeginPrefix
        }Theorem 1${
          DEFAULT_PROFILES[DEFAULT_SETTINGS.profile].body.proof
            .linkedBeginSuffix
    ...
    contentEl.insertBefore(
      linkedProofHeading,
      this.settingRefs.linkedBeginPrefix.settingEl
    );
  }
  onClose() {
...
var ConfirmProfileDeletionModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    ...
    contentEl.createDiv({
      text: `Are you sure you want to delete the profile "${this.id}"?`,
    });
    const buttonContainerEl = contentEl.createDiv({
      cls: "math-booster-button-container",
    });
    new import_obsidian3.ButtonComponent(buttonContainerEl)
...
var AddProfileModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    ...
    const addProfileEl = contentEl.createDiv({
      cls: "math-booster-add-profile",
    });
    new import_obsidian3.TextComponent(addProfileEl)
      .setPlaceholder("Enter name...")
      .onChange((value) => {
        id = value;
      });
    const buttonContainerEl = addProfileEl.createDiv({
      cls: "math-booster-button-container",
    });
    new import_obsidian3.ButtonComponent(buttonContainerEl)
      .setButtonText("Add")
...
var UpdateProfileModal = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    ...
    contentEl.createDiv({
      text: `The following ${
        this.affected.length > 1 ? this.affected.length : ""
      } local setting${
        this.affected.length > 1 ? "s are" : " is"
      } affected by the deletion of profile "${
        this.deletedID
      }." Select a new profile to be applied for them.`,
    });
    ...
    for (const path of this.affected) {
      listEl.createEl("li", {
        text: path == VAULT_ROOT ? "(Vault root)" : path,
      });
    }
  }
  onClose() {
...
function makeIdOfCopy(oldID, profiles) {
  ...
  const numbers = ids
    .map((id) => {
      var _a, _b;
      return (_b =
        (_a = id.slice(oldID.length + 1).match(/\(([1-9][0-9]*)\)/)) == null
          ? void 0
          : _a[1]) != null
        ? _b
        : "0";
...
var NUMBER_STYLES = ["arabic", "alph", "Alph", "roman", "Roman"];
var THEOREM_CALLOUT_STYLES = ["Custom", "Plain", "Framed", "MathWiki", "Vivid"];
...
var SEARCH_METHODS = ["Fuzzy", "Simple"];
...
function getFile(app) {
  var _a;
  return (_a = app.workspace.getActiveFile()) != null
    ? _a
    : app.vault.getRoot();
}
function getSectionCacheFromPos(cache, pos, type) {
  if (cache.sections) {
    const sectionCache = Object.values(cache.sections).find(
      (sectionCache2) =>
        sectionCache2.type == type &&
        (sectionCache2.position.start.offset == pos ||
          sectionCache2.position.end.offset == pos)
    );
    return sectionCache;
  }
}
...
function getSectionCacheFromMouseEvent(event, type, view, cache) {
  var _a;
  const pos =
    (_a = view.posAtCoords(event)) != null
      ? _a
      : view.posAtCoords(event, false);
  return getSectionCacheFromPos(cache, pos, type);
}
function getProperty(app, file, name) {
  var _a, _b;
  return (_b =
    (_a = app.metadataCache.getFileCache(file)) == null
      ? void 0
      : _a.frontmatter) == null
    ? void 0
    : _b[name];
...
function getPropertyOrLinkTextInProperty(app, file, name) {
  var _a, _b;
  return (_b =
    (_a = getPropertyLink(app, file, name)) == null ? void 0 : _a.link) != null
    ? _b
    : getProperty(app, file, name);
}
...
function getMarkdownSourceViewEl(view) {
  ...
  const firstCandidate =
    (_a = view.editor.cm) == null ? void 0 : _a.dom.parentElement;
  if (firstCandidate) return firstCandidate;
  const secondCandidate = view.previewMode.containerEl.previousSibling;
  if (
    secondCandidate instanceof HTMLElement &&
    secondCandidate.matches(".markdown-source-view")
  ) {
  ...
  return (
    ((_b =
      (_a = el.parentElement) == null
        ? void 0
        : _a.classList.contains("print")) != null
      ? _b
      : false) && el.matches(".markdown-preview-view.markdown-rendered")
  );
}
function isPluginOlderThan(plugin, version) {
  return (
    plugin.manifest.version.localeCompare(version, void 0, { numeric: true }) <
    0
  );
}
function getModifierNameInPlatform(mod) {
  if (mod == "Mod") {
    return import_obsidian5.Platform.isMacOS ||
      import_obsidian5.Platform.isIosApp
      ? "\u2318"
      : "ctrl";
  }
  if (mod == "Shift") {
  ...
  if (mod == "Meta") {
    return import_obsidian5.Platform.isMacOS ||
      import_obsidian5.Platform.isIosApp
      ? "\u2318"
      : import_obsidian5.Platform.isWin
      ? "win"
      : "meta";
  }
  return "ctrl";
  ...
  if (settings.number) {
    if (settings.number == "auto") {
      if (settings._index !== void 0) {
        ...
        const style =
          (_b = settings.numberStyle) != null
            ? _b
            : DEFAULT_SETTINGS.numberStyle;
        title += ` ${getNumberPrefix(plugin.app, file, settings)}${CONVERTER[
          style
        ](num)}${settings.numberSuffix}`;
      }
    } else {
...
function getNumberPrefix(app, file, settings) {
  ...
  const source = settings.inferNumberPrefixFromProperty
    ? getPropertyOrLinkTextInProperty(
        app,
        file,
        settings.inferNumberPrefixFromProperty
      )
    : file.basename;
  if (settings.inferNumberPrefix && source) {
    return (_a = inferNumberPrefix(source, settings.inferNumberPrefixRegExp)) !=
      null
      ? _a
      : "";
  }
...
function getEqNumberPrefix(app, file, settings) {
  ...
  const source = settings.inferEqNumberPrefixFromProperty
    ? getPropertyOrLinkTextInProperty(
        app,
        file,
        settings.inferEqNumberPrefixFromProperty
      )
    : file.basename;
  if (settings.inferEqNumberPrefix && source) {
    const prefix =
      (_a = inferNumberPrefix(source, settings.inferEqNumberPrefixRegExp)) !=
      null
        ? _a
        : "";
    return prefix;
...
function formatLabel(settings) {
  if (settings.label) {
    return (
      ...
      THEOREM_LIKE_ENVs[settings.type].prefix +
      ":" +
      settings.label
    );
  }
  ...
  makeSettingPane() {
    ...
    new import_obsidian8.Setting(contentEl)
      ...
      .addDropdown((dropdown) => {
        ...
        const numberSetting = new import_obsidian8.Setting(contentEl).setName(
          "Number"
        );
        const numberSettingDescList = numberSetting.descEl.createEl("ul");
        numberSettingDescList.createEl("li", {
          text: '"auto" - automatically numbered',
        });
        numberSettingDescList.createEl("li", { text: "blank - unnumbered" });
        numberSettingDescList.createEl("li", {
        ...
        numberSetting.addText((text) => {
          var _a3;
          text.setValue(
            (_a3 = this.defaultSettings.number) != null
              ? _a3
              : this.defaultSettings.numberDefault
          );
        ...
        const titlePane = new import_obsidian8.Setting(contentEl)
          .setName("Title")
          .setDesc("You can include inline math in the title.");
        const labelPane = this.plugin.extraSettings.setLabelInModal
          ? new import_obsidian8.Setting(contentEl).setName("Pandoc label")
          : void 0;
        const labelPrefixEl =
          labelPane == null
            ? void 0
            : labelPane.controlEl.createDiv({
                text:
                  THEOREM_LIKE_ENVs[this.settings.type].prefix +
                  ":" +
                  ((_a2 = this.defaultSettings.labelPrefix) != null ? _a2 : ""),
              });
        titlePane.addText((text) => {
          ...
          text.setPlaceholder("Ex) $\\sigma$-algebra").onChange((value) => {
            ...
            if (this.plugin.extraSettings.setLabelInModal) {
              let labelInit = this.settings.title
                .replaceAll(" ", "-")
                .replaceAll("'s", "")
                .toLowerCase();
    ...
    addFoldOptionSetting(
      contentEl,
      "Collapse",
      (fold) => {
        this.settings.fold = fold;
      },
      (_a = this.defaultSettings.fold) != null
        ? _a
        : this.plugin.extraSettings.foldDefault
    );
  }
};
var SettingsHelper = class extends import_obsidian8.Component {
  ...
  addDropdownSetting(
    ...
    const setting = new import_obsidian8.Setting(this.contentEl).setName(
      prettyName
    );
    if (description) {
  ...
  addTextSetting(name, prettyName, description, number = false) {
    ...
    setting.addText((text) => {
      ...
      text
        .setPlaceholder(
          String((_a = this.defaultSettings[name]) != null ? _a : "")
        )
        .setValue(String((_b = this.settings[name]) != null ? _b : ""))
        .onChange((value) => {
          if (number) {
            Object.assign(this.settings, { [name]: +value });
          } else {
  ...
  addSliderSetting(name, limits, prettyName, description) {
    ...
    setting.addSlider((slider) => {
      ...
      slider
        ...
        .setDynamicTooltip()
        .setValue(this.defaultSettings[name]);
...
var MathContextSettingsHelper = class extends SettingsHelper {
  ...
  onload() {
    ...
    this.addHeading("Search & link auto-completion - general").then(
      async (setting) => {
    ...
    this.addToggleSetting(
      "insertSpace",
      "Append whitespace after inserted link"
    );
  }
  addProfileSetting(defaultValue) {
    ...
    new import_obsidian8.ButtonComponent(profileSetting.controlEl)
...
var ExtraSettingsHelper = class extends SettingsHelper {
  onload() {
    ...
    this.addToggleSetting(
      ...
      "Render equations inside callouts & add multi-line equation support to blockquotes",
      void 0,
      ...
      await import_obsidian8.MarkdownRenderer.render(
        this.plugin.app,
        "**NOTE:** This feature is planned to be removed from this plugin, and instead, it will be available as a separate plugin [**Better Math in Callouts & Blockquotes**](https://github.com/RyotaUshio/obsidian-math-in-callout), featuring a bunch of improvements. Currently awaiting for approval by the Obsidian team.",
        setting.descEl,
        "",
        this
      );
    ...
    this.addToggleSetting(
      ...
      `For example, you can replace a pair of inline codes \`${
        DEFAULT_SETTINGS.beginProof
      }\` & \`${DEFAULT_SETTINGS.endProof}\` with "${
    ...
    this.addToggleSetting(
      ...
      `Show the instruction for the modifier key at the bottom of suggestion box. Changing this option requires to reloading ${this.plugin.manifest.name} to take effect.`
    );
    ...
    list.createEl("li", {
      text: "Meta is Cmd on MacOS and Win key on Windows.",
    });
    this.addDropdownSetting(
      "suggestLeafOption",
      LEAF_OPTIONS,
    ...
    this.addHeading(
      "Enhance Obsidian's built-in link auto-completion (experimental)"
    ).setDesc(
    ...
    this.addToggleSetting("renderEquationinBuiltin", "Render equation").then(
      async (setting) => {
        setting.descEl.addClass("math-booster-new-feature");
        await import_obsidian8.MarkdownRenderer.render(
          this.plugin.app,
    ...
    this.addHeading(
      "Configure this plugin's custom editor link auto-completion"
    ).setDesc(
    ...
    this.addHeading("From entire vault", [
...
function addFoldOptionSetting(el, name, onChange, defaultValue) {
  return new import_obsidian8.Setting(el)
    .setName(name)
    .addDropdown((dropdown) => {
      ...
      dropdown.setValue(
        defaultValue != null ? defaultValue : DEFAULT_EXTRA_SETTINGS.foldDefault
      );
  ...
  obsidianLink() {
    ...
    if (this.type == "header")
      return (
        escaped +
        "#" +
        ((_a = this.subpath) == null ? void 0 : _a.replace("|", "\\|"))
      );
    if (this.type == "block")
      return (
        escaped +
        "#^" +
        ((_b = this.subpath) == null ? void 0 : _b.replace("|", "\\|"))
      );
    else return escaped;
  }
...
((Literals2) => {
  ...
  function toString(
    field,
    setting = Literals2.DEFAULT_TO_STRING,
    recursive = false
  ) {
  ...
  function compare(val1, val2, linkNormalizer) {
    ...
    switch (wrap1.type) {
      ...
      case "link":
        ...
        return ((_a = link1.subpath) != null ? _a : "").localeCompare(
          (_b = link2.subpath) != null ? _b : ""
        );
      case "array":
        let f1 = wrap1.value;
        let f2 = wrap2.value;
...
var _MarkdownPage = class _MarkdownPage {
  ...
  getBlockByLineNumber(line) {
    const section = this.$sections.find(
      (section2) =>
        section2.$position.start <= line && line <= section2.$position.end
    );
    const block =
      section == null
        ? void 0
        : section.$blocks.find(
            (block2) =>
              block2.$position.start <= line && line <= block2.$position.end
          );
    return block;
  }
  getBlockByOffset(offset) {
    for (const section of this.$sections) {
      for (const block of section.$blocks) {
        if (
          block.$pos.start.offset <= offset &&
          offset <= block.$pos.end.offset
        )
          return block;
      }
    }
  ...
  static isMarkdownPage(object) {
    return (
      object !== void 0 && "$typename" in object && object.$typename === "Page"
    );
  }
};
...
var _MarkdownSection = class _MarkdownSection {
  ...
  static isMarkdownSection(object) {
    return (
      object !== void 0 &&
      "$typename" in object &&
      object.$typename === "Section"
    );
  }
};
/** All of the types that a markdown section is. */
_MarkdownSection.TYPES = [
  "markdown",
  "section",
  LINKABLE_TYPE,
  LINKBEARING_TYPE,
...
function getIO(plugin, file, activeMarkdownView) {
  activeMarkdownView =
    activeMarkdownView != null
      ? activeMarkdownView
      : plugin.app.workspace.getActiveViewOfType(import_obsidian9.MarkdownView);
  if (
    activeMarkdownView &&
    activeMarkdownView.file == file &&
    isEditingView(activeMarkdownView)
  ) {
...
function staticifyEqNumber(plugin, file) {
  ...
  if (!MarkdownPage.isMarkdownPage(page)) {
    new import_obsidian10.Notice(
      `Failed to fetch the metadata of file ${file.path}.`
    );
    return;
  }
...
async function insertBlockIdIfNotExist(
  plugin,
  targetFile,
  cache,
  block,
  ...
  if (!(cache == null ? void 0 : cache.sections)) return;
...
function insertDisplayMath(editor) {
  ...
  const head =
    (_a = nonQuoteMatch == null ? void 0 : nonQuoteMatch.index) != null
      ? _a
      : line.length;
  const quoteLevel =
    (_c =
      (_b = line.slice(0, head).match(/>\s*/g)) == null ? void 0 : _b.length) !=
    null
      ? _c
      : 0;
  let insert =
...
async function rewriteTheoremCalloutFromV1ToV2(plugin, file) {
  ...
  await app.vault.process(file, (data) =>
    convertTheoremCalloutFromV1ToV2(data, page)
  );
}
var convertTheoremCalloutFromV1ToV2 = (data, page) => {
  ...
  for (const section of page.$sections) {
    for (const block of section.$blocks) {
      ...
      const newHeadLines = [
        generateTheoremCalloutFirstLine({
...
function generateTheoremCalloutFirstLine(config) {
  ...
  const metadata =
    config.number === "auto"
      ...
      ? "|*"
      : `|${config.number}`;
  let firstLine = `> [!${config.type}${metadata}]${
    (_a = config.fold) != null ? _a : ""
...
function isTheoremCallout(plugin, type) {
  if (plugin.extraSettings.excludeExampleCallout && type === "example")
    return false;
  return (
    THEOREM_LIKE_ENV_IDs.includes(type) ||
    THEOREM_LIKE_ENV_PREFIXES.includes(type) ||
    type === "math"
  );
}
function insertProof(plugin, editor, context) {
  ...
  const settings = resolveSettings(
    void 0,
    plugin,
    (_a = context.file) != null ? _a : getFile(plugin.app)
  );
...
var ContextSettingModal = class extends MathSettingModal {
  ...
  onOpen() {
    ...
    const defaultSettings = this.file.parent
      ? resolveSettings(void 0, this.plugin, this.file.parent)
      : DEFAULT_SETTINGS;
...
var FileSuggestModal = class extends import_obsidian12.FuzzySuggestModal {
  ...
  filterCallback(abstractFile) {
    if (
      abstractFile instanceof import_obsidian12.TFile &&
      abstractFile.extension != "md"
    ) {
      return false;
    }
    if (
      abstractFile instanceof import_obsidian12.TFolder &&
      abstractFile.isRoot()
    ) {
      return false;
    }
  ...
  async newDisplay() {
    ...
    if (this.plugin.excludedFiles.length) {
      ...
      for (const path of this.plugin.excludedFiles) {
        ...
        new import_obsidian12.Setting(item)
          .setName(path)
          .addExtraButton((btn) => {
...
var MathSettingTab = class extends import_obsidian14.PluginSettingTab {
  ...
  display() {
    ...
    const numberingHeading =
      this.containerEl.querySelector(".equation-heading");
    this.containerEl.insertBefore(heading.settingEl, numberingHeading);
    ...
    new import_obsidian14.Setting(containerEl)
      ...
      .addButton((btn) => {
        btn.setButtonText("Search files & folders").onClick(() => {
          new LocalContextSettingsSuggestModal(
            this.app,
            this.plugin,
            this
          ).open();
...
var CleverefProvider = class extends Provider {
  ...
  provide(parsedLinktext, targetFile, targetSubpathResult) {
    ...
    if (!subpath) return (_a = page.$refName) != null ? _a : null;
    const processedPath = path
      ? (_b = page.$refName) != null
        ? _b
        : path
      : "";
...
async function renderMarkdown(markdown, sourcePath, component) {
  const el = createSpan();
  await import_obsidian15.MarkdownRenderer.renderMarkdown(
    markdown,
    el,
    sourcePath,
    component
...
function readTheoremCalloutSettings(line, excludeExample = false) {
  var _a, _b, _c, _d, _e;
  const rawSettings =
    (_a = line.match(THEOREM_CALLOUT_PATTERN)) == null ? void 0 : _a.groups;
  ...
  if (excludeExample && type === "example") return void 0;
  if (type.length <= 4) {
    type = THEOREM_LIKE_ENV_PREFIX_ID_MAP[type];
  }
  const number = parseTheoremCalloutMetadata(
    (_b = rawSettings.number) != null ? _b : ""
  );
  let title = (_c = rawSettings.title) == null ? void 0 : _c.trim();
  if (title === "") title = void 0;
  const fold =
    (_e = (_d = rawSettings.fold) == null ? void 0 : _d.trim()) != null
      ? _e
      : "";
...
function _readTheoremCalloutSettings(callout, excludeExample = false) {
  ...
  if (excludeExample && callout.type === "example") return void 0;
...
// src/theorem-callouts/renderer.ts
var createTheoremCalloutPostProcessor =
  (plugin) => async (element, context) => {
    var _a;
    const file =
      (_a = plugin.app.vault.getAbstractFileByPath(context.sourcePath)) != null
        ? _a
    ...
    for (const calloutEl of element.querySelectorAll(`.callout`)) {
      ...
      if (isTheoremCallout(plugin, type)) {
        ...
        const theoremCallout = new TheoremCalloutRenderer(
          calloutEl,
          context,
          file,
          plugin
...
var TheoremCalloutRenderer = class _TheoremCalloutRenderer extends import_obsidian16.MarkdownRenderChild {
  constructor(containerEl, context, file, plugin) {
    ...
    this.index = plugin.indexManager.index;
    this.addChild(
      (this.observer = new MutationObservingChild(
        this.containerEl,
        (mutations) => {
          for (const mutation of mutations) {
            if (
              mutation.oldValue !==
              this.containerEl.getAttribute("data-theorem-index")
            ) {
              this.update();
            }
          }
    ...
    this.registerEvent(
      this.plugin.indexManager.on("index-updated", (file2) => {
  ...
  isLivePreview() {
    return (
      this.containerEl.closest("[src]") === null &&
  ...
  update() {
    const existingMainTitleEl = this.containerEl.querySelector(
    ...
    if (existingMainTitleEl && this.isLivePreview()) {
      ...
      const newMainTitle = formatTitleWithoutSubtitle(
        this.plugin,
        this.file,
        resolvedSettings
      );
    ...
    if (
      !this.info ||
      _TheoremCalloutRenderer.areDifferentInfo(info, this.info)
    ) {
  ...
  correctEmbedOrHoverPagePreview(block, info) {
    var _a, _b;
    let linktext =
      (_a = this.containerEl.closest("[src]")) == null
        ? void 0
        : _a.getAttribute("src");
    if (!linktext) {
      const hoverEl = this.containerEl.closest(
        ".hover-popover:not(.hover-editor)"
      );
      if (hoverEl) {
    ...
    if (linktext) {
      ...
      const { file, subpathResult: result } =
        (_b = resolveLinktext(this.app, linktext, this.context.sourcePath)) !=
        null
          ? _b
          : {};
  ...
  findTheoremCalloutBlock(lineOffset = 0) {
    ...
    if (!info) return null;
    const block =
      (_a = page.getBlockByLineNumber(info.lineStart + lineOffset)) != null
        ? _a
  ...
  getTheoremCalloutInfoFromEl() {
    ...
    let theoremSubtitleEl = this.containerEl.querySelector(
      ".theorem-callout-subtitle"
    );
    if (theoremSubtitleEl === null) {
      const titleInnerEl = this.containerEl.querySelector(
        ".callout-title-inner"
      );
      if (titleInnerEl == null ? void 0 : titleInnerEl.childNodes.length) {
        if (
          ...
          titleInnerEl.textContent !==
            capitalize(THEOREM_LIKE_ENV_ID_PREFIX_MAP[settings.type])
        ) {
          theoremSubtitleEl = createSpan({ cls: "theorem-callout-subtitle" });
          theoremSubtitleEl.replaceChildren(
            "(",
            ...titleInnerEl.childNodes,
            ")"
          );
        }
      }
    ...
    return {
      ...
      theoremMainTitle: formatTitleWithoutSubtitle(
        this.plugin,
        this.file,
        resolvedSettings
      ),
  ...
  addCssClasses(info) {
    ...
    const profile =
      this.plugin.extraSettings.profiles[resolvedSettings.profile];
  ...
  addEditButton() {
    ...
    const button = new import_obsidian16.ExtraButtonComponent(this.containerEl)
      .setIcon("settings-2")
      ...
      new TheoremCalloutModal(
        ...
        async (settings) => {
          if (lineNumber !== void 0) {
            await io.setLine(
              lineNumber,
              generateTheoremCalloutFirstLine(settings)
            );
          } else {
        ...
        readTheoremCalloutSettings(
          line,
          this.plugin.extraSettings.excludeExampleCallout
        )
      ).open();
    });
  }
  getLineNumber(event) {
    ...
    if (
      isEditingView(view) &&
      this.file.path === ((_b = view.file) == null ? void 0 : _b.path) &&
      view.editor.cm
    ) {
      let sec =
        (_c = getSectionCacheOfDOM(
          this.containerEl,
          "callout",
          view.editor.cm,
          cache
...
function readSettingsFromEl(calloutEl) {
  var _a;
  let type =
    (_a = calloutEl.getAttribute("data-callout")) == null
      ? void 0
      : _a.trim().toLowerCase();
...
var createTheoremCalloutNumberingViewPlugin = (plugin) =>
...
var createEquationNumberProcessor = (plugin) => async (el, ctx) => {
  ...
  if (!(sourceFile instanceof import_obsidian19.TFile)) return;
  const mjxContainerElements = el.querySelectorAll(
...
function preprocessForPdfExport(plugin, el, ctx) {
  try {
    const topLevelMathDivs = el.querySelectorAll(
...
var EquationNumberRenderer = class extends import_obsidian19.MarkdownRenderChild {
  constructor(containerEl, plugin, file, context) {
    ...
    this.registerEvent(
      this.plugin.indexManager.on("index-initialized", () => {
    ...
    this.registerEvent(
      this.plugin.indexManager.on("index-updated", (file2) => {
        setTimeout(() => {
          if (file2.path === this.file.path) this.update();
        });
      })
    );
  }
...
function createEquationNumberPlugin(plugin) {
  ...
  plugin.registerEvent(
    plugin.indexManager.on("index-updated", (file) => {
      app.workspace.iterateAllLeaves((leaf) => {
        var _a, _b;
        if (
          leaf.view instanceof import_obsidian21.MarkdownView &&
          ((_a = leaf.view.file) == null ? void 0 : _a.path) === file.path &&
          leaf.view.getMode() === "source"
        ) {
          (_b = leaf.view.editor.cm) == null
            ? void 0
            : _b.dispatch({ effects: forceUpdateEffect.of(null) });
        }
      });
    })
  );
  return import_view2.ViewPlugin.fromClass(
    class {
      ...
      update(update2) {
        if (!this.file) this.updateFile(update2.state);
        if (!this.file) return;
        if (
          update2.transactions.some((tr) =>
            tr.effects.some((effect) => effect.is(forceUpdateEffect))
          )
        ) {
          this.settings = resolveSettings(void 0, plugin, this.file);
          this.updatePage(this.file).then((updatedPage) =>
            this.updateEquationNumber(update2.view, updatedPage)
          );
        } else if (update2.geometryChanged) {
          if (this.page) this.updateEquationNumber(update2.view, this.page);
          else
            this.updatePage(this.file).then((updatedPage) =>
              this.updateEquationNumber(update2.view, updatedPage)
            );
      ...
      async updateEquationNumber(view, page) {
        var _a, _b;
        const mjxContainerElements = view.contentDOM.querySelectorAll(
          ':scope > .cm-embed-block.math > mjx-container.MathJax[display="true"]'
        );
        for (const mjxContainerEl of mjxContainerElements) {
          const mightBeClosingDollars =
            (_b =
              (_a = mjxContainerEl.parentElement) == null
                ? void 0
                : _a.previousElementSibling) == null
              ? void 0
              : _b.lastElementChild;
          const isBeingEdited =
            mightBeClosingDollars == null
              ? void 0
              : mightBeClosingDollars.matches("span.cm-formatting-math-end");
          ...
          if (
            mjxContainerEl.getAttribute("data-equation-print-name") !==
            block.$printName
          ) {
...
var createTheoremCalloutsField = (plugin) =>
  import_state2.StateField.define({
    create(state) {
...
function getTheoremCalloutInfos(plugin, state, doc, from, init) {
  ...
  const ranges = [];
  const tree =
    (_a = (0, import_language.ensureSyntaxTree)(state, doc.length)) != null
      ? _a
      : (0, import_language.syntaxTree)(state);
  ...
  tree.iterate({
    ...
    enter(node) {
      ...
      const value = new TheoremCalloutInfo(
        settings.number === "auto" ? theoremIndex++ : null
      );
    ...
    if (this.info.display) {
      const containerEl = createDiv({
        cls: [
          ...
          "HyperMD-quote-lazy",
          "cm-line",
          "math-booster-preview-container",
        ],
      });
      ...
      const editButton = new import_obsidian23.ExtraButtonComponent(
        cmEmbedBlockEl
      )
        .setIcon("code-2")
      ...
      editButton.extraSettingsEl.addEventListener("click", (ev) => {
        ev.stopPropagation();
        view.dispatch({
          selection: { anchor: this.info.from + 2, head: this.info.to - 2 },
        });
      });
      editButton.extraSettingsEl.classList.add(
        "math-booster-preview-edit-button"
      );
      return containerEl;
    }
...
var MathInfo = class extends import_state3.RangeValue {
  ...
  async render() {
    this.mathEl = (0, import_obsidian23.renderMath)(
      this.mathText,
      this.display
    );
  }
...
function buildMathInfoSet(state) {
  ...
  tree.iterate({
    enter(node) {
      ...
      if (
        nodeText(node, state).trim() == ">" &&
        node.from < quoteContentStart
      ) {
        return;
      }
      if (insideMath) {
        ...
        const match =
          (_c = node.node.parent) == null ? void 0 : _c.name.match(BLOCKQUOTE);
        if (match) {
...
var mathPreviewInfoField = import_state3.StateField.define({
  ...
  update(prev, transaction) {
    ...
    if (!prev.mathInfoSet.size) {
      ...
      if (
        !prev.isInCalloutsOrQuotes ||
        (prev.hasOverlappingMath && !hasOverlappingMath)
      ) {
        mathInfoSet = buildMathInfoSet(transaction.state);
        rerendered = true;
    ...
    return {
      mathInfoSet,
      isInCalloutsOrQuotes,
      hasOverlappingMath,
      hasOverlappingDisplayMath,
      rerendered,
    };
  },
});
  ...
  class {
    ...
    update(update2) {
      if (
        ...
        overlapStateChanged(update2.state)
      ) {
    ...
    buildDecorations(view) {
      ...
      for (const { from, to } of view.visibleRanges) {
        field.mathInfoSet.between(from, to, (from2, to2, value) => {
          if (
            value.insideCallout &&
            !value.display &&
            !hasOverlap(range, { from: from2, to: to2 })
          ) {
            builder.add(from2, to2, value.toDecoration("replace"));
          }
        });
      }
    ...
    impl(view) {
      ...
      for (const vanillaObsidianMathPreview of view.contentDOM.querySelectorAll(
        ...
        if (
          rangeSetSome(
            field.mathInfoSet,
            (info) => info.from - 1 <= pos && pos <= info.to
          )
        ) {
          const cmWidgetBuffer = vanillaObsidianMathPreview.previousSibling;
          if (
            cmWidgetBuffer instanceof HTMLElement &&
            cmWidgetBuffer.matches(".cm-widget-buffer")
          ) {
...
((Transferable2) => {
  function transferable(value2) {
    if (value2 instanceof Map) {
      ...
      for (let val of value2) copied.add(transferable(val));
      return copied;
    }
  ...
  function value(transferable2) {
    if (transferable2 === null) {
      ...
      for (let val of transferable2) real.add(value(val));
      return real;
      ...
      if ("$transfer-type" in transferable2) {
        switch (transferable2["$transfer-type"]) {
          ...
          default:
            throw Error(
              `Unrecognized transfer type '${transferable2["$transfer-type"]}'`
            );
        }
      }
      let result = {};
...
var _InvertedIndex = class _InvertedIndex {
  ...
  get(value) {
    var _a;
    return (_a = this.inverted.get(value)) != null
      ? _a
      : _InvertedIndex.EMPTY_SET;
  }
...
var MathIndex = class {
  ...
  _recursiveStore(object, revision, substorer, parent) {
    ...
    substorer == null
      ...
      : substorer(object, (incoming, subindex) =>
          this._recursiveStore(incoming, revision, subindex, object)
        );
  }
  ...
  _index(object) {
    ...
    if (
      object.$types.contains(LINKBEARING_TYPE) &&
  ...
  updateNames(file) {
    ...
    while ((block = this.load(`${file.path}/block${blockOrdinal++}`))) {
      if (TheoremCalloutBlock.isTheoremCalloutBlock(block)) {
        ...
        const mainTitle = formatTitleWithoutSubtitle(
          this.plugin,
          file,
          resolvedSettings
        );
        const refName = this.formatMathLink(
          file,
          resolvedSettings,
          "refFormat"
        );
        ...
        if (block.$manualTag) {
          ...
          (block.$link && this.isLinked(block))
        ) {
          block.$index = equationCount;
          printName =
            "(" +
            eqPrefix +
            CONVERTER[settings.eqNumberStyle](
              equationNumberInit + equationCount
            ) +
            eqSuffix +
            ")";
          equationCount++;
    ...
    if (
      this.plugin.extraSettings.setOnlyTheoremAsMain &&
      theorems.length == 1
    ) {
    ...
    if (MarkdownPage.isMarkdownPage(page)) {
      if (mainTheorem) {
        const resolvedSettings = Object.assign(
          {},
          settings,
          mainTheorem.$settings
        );
        resolvedSettings._index = mainTheorem.$index;
        if (
          !resolvedSettings.ignoreMainTheoremCalloutWithoutTitle ||
          mainTheorem.$theoremSubtitle
        )
          page.$refName = this.formatMathLink(
            file,
            resolvedSettings,
            "noteMathLinkFormat"
          );
  ...
  formatMathLink(file, resolvedSettings, key) {
    ...
    const typePlusNumber = formatTitleWithoutSubtitle(
      this.plugin,
      file,
      resolvedSettings
    );
...
function iterableExists(object, key) {
  return (
    key in object && object[key] !== void 0 && Symbol.iterator in object[key]
  );
}
...
var MathIndexManager = class extends import_obsidian25.Component {
  constructor(plugin, settings) {
    ...
    this.addChild(
      (this.importer = new MathImporter(
        this.plugin,
        app.vault,
        app.metadataCache,
        () => {
          return {
            ...
            utilization: Math.max(
              0.1,
              Math.min(1, settings.importerUtilization)
            ),
          };
        }
      ))
  ...
  initialize() {
    this.registerEvent(
      this.metadataCache.on("resolve", (file) => this.updateLinked(file))
    );
  ...
  async rename(file, oldPath) {
    ...
    this.plugin.settings[file.path] = structuredClone(
      this.plugin.settings[oldPath]
    );
  ...
  async reload(file) {
    ...
    if (result.type === "error") {
      ...
      const parsed = MarkdownPage.from(result.result, (link) => {
        const rpath = this.metadataCache.getFirstLinkpathDest(
          link.path,
          result.result.$path
        );
    ...
    throw new Error(
      "Encountered unrecognized import result type: " + result.type
    );
  }
...
var _MathIndexInitializer = class _MathIndexInitializer extends import_obsidian25.Component {
  ...
  runNext() {
    if (
      !this.active ||
      this.current.length >= _MathIndexInitializer.BATCH_SIZE
    ) {
      return;
    }
    ...
    if (next) {
      ...
      this.manager.vault
        .getMarkdownFiles()
        .forEach((file) => this.manager.index.updateNames(file));
  ...
  async init(file) {
    try {
      ...
      console.log(
        `${this.manager.plugin.manifest.name}: Failed to import file: `,
        ex
      );
...
var DependencyNotificationModal = class extends import_obsidian27.Modal {
  ...
  async onOpen() {
    ...
    contentEl.empty();
    titleEl.setText(
      `${this.plugin.manifest.name} ${this.plugin.manifest.version}`
    );
  ...
  showDependencies() {
    ...
    for (const depenedency of Object.values(this.plugin.dependencies)) {
      ...
      const isValid =
        depPlugin && !isPluginOlderThan(depPlugin, depenedency.version);
...
To fully enjoy version 2, click the button below to convert the old theorem format to the new one. Alternatively, you can do it later by running the command "Migrate from version 1".`,
    ...
    new import_obsidian27.Setting(this.contentEl)
      .addButton((button) => {
        button
    ...
    const descEl = this.contentEl.createDiv({
      cls: "math-booster-version-2-release-note-modal",
    });
...
var MigrationModal = class extends import_obsidian27.Modal {
  ...
  async onOpen() {
    ...
    contentEl.empty();
    (_a = modalEl.querySelector(".modal-close-button")) == null
      ? void 0
      : _a.remove();
...
`,
    ...
    if (
      ...
      !this.plugin.indexManager.initialized
    ) {
    ...
    contentEl.empty();
    const waitForCacheRefresh = new import_obsidian27.Setting(
      contentEl
    ...
    const converting = new import_obsidian27.Setting(contentEl).setName(
      "Converting..."
    );
    await new Promise((resolve) => {
...
var MathSearchModal = class extends import_obsidian29.SuggestModal {
  constructor(plugin) {
    ...
    new import_obsidian29.Setting(this.topEl)
      .setName("Query type")
      .addDropdown((dropdown) => {
        dropdown
          .addOption("both", "Theorems and equations")
    ...
    this.dvQueryField = new import_obsidian29.Setting(this.topEl)
      ...
      .addTextArea((text) => {
        ...
        text
          .setValue(this.plugin.extraSettings.searchModalDvQuery)
          .setPlaceholder("LIST ...")
          .onChange((dvQuery) => {
  ...
  resetCore() {
    ...
    if (this.range === "dataview") {
      if (!core) {
        ...
        this.dvQueryField
          .setDesc("Retry after enabling Dataview.")
...
var MathSearchCore = class {
  ...
  setScope() {
    this.scope.register(
      [this.plugin.extraSettings.modifierToJump],
      "Enter",
      () => {
    ...
    this.scope.register(
      [this.plugin.extraSettings.modifierToNoteLink],
      "Enter",
      () => {
        const item = this.parent.getSelectedItem();
    ...
    if (this.plugin.extraSettings.showModifierInstruction) {
      this.parent.setInstructions([
        ...
        {
          command: `${getModifierNameInPlatform(
            this.plugin.extraSettings.modifierToNoteLink
          )} + \u21B5`,
  ...
  gradeSuggestions(ids, query) {
    var _a;
    const callback = (
      this.plugin.extraSettings.searchMethod == "Fuzzy"
        ? import_obsidian30.prepareFuzzySearch
        : import_obsidian30.prepareSimpleSearch
    )(query);
    const results = [];
    for (const id of ids) {
      ...
      if (block.$type === "theorem") {
        ...
        if (this.plugin.extraSettings.searchLabel) {
          ...
          if (file instanceof import_obsidian30.TFile) {
            ...
            text += ` ${
              (_a = formatLabel(resolvedSettings)) != null ? _a : ""
            }`;
          }
        }
  ...
  renderSuggestion(block, el) {
    ...
    if (block.$type === "equation") {
      if (this.plugin.extraSettings.renderMathInSuggestion) {
        const mjxContainerEl = (0, import_obsidian30.renderMath)(
          block.$mathText,
          true
        );
  ...
  async selectSuggestionImpl(block, insertNoteLink) {
    const context = this.parent.getContext();
    if (!context) return;
    const fileContainingBlock = this.app.vault.getAbstractFileByPath(
      block.$file
    );
    ...
    const settings = resolveSettings(void 0, this.plugin, file);
    let success = false;
    const result = await insertBlockIdIfNotExist(
      this.plugin,
      fileContainingBlock,
      cache,
      block
    ...
    if (!success) {
      new import_obsidian30.Notice(
        `${this.plugin.manifest.name}: Failed to read cache. Retry again later.`,
        5e3
      );
    }
  }
...
var LinkAutocomplete = class extends import_obsidian32.EditorSuggest {
  ...
  setTriggers() {
    ...
    if (this.plugin.extraSettings.enableTheoremSuggestRecentNotes) {
      unsortedTriggers[
        this.plugin.extraSettings.triggerTheoremSuggestRecentNotes
      ] = { range: "recent", queryType: "theorem" };
    }
    if (this.plugin.extraSettings.enableEquationSuggestRecentNotes) {
      unsortedTriggers[
        this.plugin.extraSettings.triggerEquationSuggestRecentNotes
      ] = { range: "recent", queryType: "equation" };
    }
    ...
    if (this.plugin.extraSettings.enableTheoremSuggestActiveNote) {
      unsortedTriggers[
        this.plugin.extraSettings.triggerTheoremSuggestActiveNote
      ] = { range: "active", queryType: "theorem" };
    }
    if (this.plugin.extraSettings.enableEquationSuggestActiveNote) {
      unsortedTriggers[
        this.plugin.extraSettings.triggerEquationSuggestActiveNote
      ] = { range: "active", queryType: "equation" };
    }
    ...
    if (this.plugin.extraSettings.enableTheoremSuggestDataview) {
      unsortedTriggers[
        this.plugin.extraSettings.triggerTheoremSuggestDataview
      ] = { range: "dataview", queryType: "theorem" };
    }
    if (this.plugin.extraSettings.enableEquationSuggestDataview) {
      unsortedTriggers[
        this.plugin.extraSettings.triggerEquationSuggestDataview
      ] = { range: "dataview", queryType: "equation" };
    }
    ...
    around(prototype, {
      renderSuggestion(old) {
        return function (item, el) {
          ...
          if (
            plugin.extraSettings.showTheoremTitleinBuiltin &&
            item.type === "block" &&
            item.node.type === "callout" &&
            isTheoremCallout(plugin, item.node.callout.type)
          ) {
            let title =
              (_b =
                (_a = item.node.children.find(
                  (child) => child.type === "callout-title"
                )) == null
                  ? void 0
                  : _a.children.map((child) => child.value).join("")) != null
                ? _b
                : "";
            const content = item.display.slice(title.length);
            const page = plugin.indexManager.index.load(item.file.path);
            if (MarkdownPage.isMarkdownPage(page)) {
              const block = page.getBlockByLineNumber(
            ...
            if (parsed) {
              ...
              const formattedTitle = formatTitle(
                plugin,
                item.file,
                resolveSettings({ type, number, title }, plugin, item.file),
                true
              );
              renderInSuggestionTitleEl(el, (suggestionTitleEl) => {
            ...
            plugin.extraSettings.renderEquationinBuiltin &&
            item.type === "block" &&
            item.node.type === "math"
          ) {
            renderInSuggestionTitleEl(el, (suggestionTitleEl) => {
              ...
              suggestionTitleEl.replaceChildren();
              suggestionTitleEl.appendChild(
                (0, import_obsidian33.renderMath)(item.node.value, true)
              );
            });
            return;
          }
...
var patchPagePreview = (plugin) => {
  ...
  plugin.register(
    // @ts-ignore
    around(
      app.internalPlugins.plugins["page-preview"].instance.constructor
        .prototype,
      {
        onLinkHover(old) {
...
function makeProofClasses(which, profile) {
  return [
    ...
    ...profile.meta.tags.map(
      (tag) => "math-booster-" + which + "-proof-" + tag
    ),
    // deprecated
    ...profile.meta.tags.map(
      (tag) => "latex-referencer-" + which + "-proof-" + tag
    ),
  ];
}
...
var ProofWidget = class extends import_view4.WidgetType {
  ...
  toDOM() {
    var _a;
    return (_a = this.containerEl) != null
      ? _a
      : (this.containerEl = this.initDOM());
  }
  ignoreEvent(event) {
    return false;
  }
...
var BeginProofWidget = class _BeginProofWidget extends ProofWidget {
  ...
  eq(other) {
    return (
      this.profile.id === other.profile.id &&
  ...
  initDOM() {
    ...
    if (display) {
      ...
      _BeginProofWidget.renderDisplay(
        el,
        display,
        this.sourcePath,
        this.plugin
...
var createProofDecoration = (plugin) =>
    ...
    class {
      ...
      makeDeco(view) {
        ...
        const settings = resolveSettings(
          void 0,
          plugin,
          file != null ? file : app.vault.getRoot()
        );
        const profile = plugin.extraSettings.profiles[settings.profile];
            ...
            enter(node) {
              ...
              if (text.startsWith(settings.beginProof)) {
                ...
                if (state.sliceDoc(node.to + 1, node.to + 2) == "@") {
                  const next =
                    (_a2 = node.node.nextSibling) == null
                      ? void 0
                      : _a2.nextSibling;
                  const afterNext =
                    (_c =
                      (_b = node.node.nextSibling) == null
                        ? void 0
                        : _b.nextSibling) == null
                      ? void 0
                      : _c.nextSibling;
                  const afterAfterNext =
                    (_f =
                      (_e =
                        (_d = node.node.nextSibling) == null
                          ? void 0
                          : _d.nextSibling) == null
                        ? void 0
                        : _e.nextSibling) == null
                      ...
                      : _f.nextSibling;
                  if (
                    (next == null ? void 0 : next.name) === LINK_BEGIN &&
                    (afterNext == null ? void 0 : afterNext.name) === LINK &&
                    (afterAfterNext == null ? void 0 : afterAfterNext.name) ===
                      LINK_END
                  ) {
  ...
  for (const code of codes) {
    ...
    if (text.startsWith(settings.beginProof)) {
      ...
      if (!rest) {
        ...
        context.addChild(
          new ProofRenderer(app, plugin, code, "begin", file, display)
        );
      }
...
function parseAtSignLink(codeEl) {
  ...
  if (afterNext) {
    if (
      ...
      next.textContent == "@" &&
      afterNext instanceof HTMLElement &&
      afterNext.matches("a.original-internal-link") &&
...
var LatexReferencer3 = class extends import_obsidian36.Plugin {
  ...
  async onload() {
    ...
    this.app.workspace.onLayoutReady(async () => {
      ...
      const v1 =
        !first &&
        ((_a = version == null ? void 0 : version.startsWith("1.")) != null
          ? _a
          : true);
    ...
    this.addChild(
      (this.indexManager = new MathIndexManager(this, this.extraSettings))
    );
    this.app.workspace.onLayoutReady(async () =>
      this.indexManager.initialize()
    );
    (window["mathIndex"] = this.indexManager.index) &&
      this.register(() => delete window["mathIndex"]);
    this.app.workspace.onLayoutReady(() => {
      this.addChild(
        addProvider(
          this.app,
          (mathLinks) => new CleverefProvider(mathLinks, this)
        )
      );
    });
    this.registerEvent(
    ...
    this.registerEvent(
      this.app.workspace.on("active-leaf-change", (leaf) => {
        if (
          ...
          import_obsidian36.MarkdownView
        ) {
          this.setProfileTagAsCSSClass(leaf.view);
        }
      })
    ...
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file) => {
        menu
          ...
          .addItem((item) => {
            item
              .setTitle(`${this.manifest.name}: Open local settings`)
              .onClick(() => {
  ...
  async loadSettings() {
    this.settings = {
      [VAULT_ROOT]: JSON.parse(JSON.stringify(DEFAULT_SETTINGS)),
    };
  ...
  setProfileTagAsCSSClass(view) {
    ...
    for (const el of [
      getMarkdownSourceViewEl(view),
      getMarkdownPreviewViewEl(view),
    ]) {
      if (el) {
        el.classList.forEach((cls) => {
          if (
            cls.startsWith("math-booster-") ||
            cls.startsWith("latex-referencer-")
          ) {
  ...
  updateEditorExtensions() {
    ...
    this.editorExtensions.push(
      (this.theoremCalloutsField = createTheoremCalloutsField(this))
    );
    ...
    document.body.toggleClass(
      "math-booster-preview-enabled",
      this.extraSettings.enableMathPreviewInCalloutAndQuote
    );
