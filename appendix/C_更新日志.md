...
- **section8/05_站点自定义_字体配置.md**：站点字体配置教程（以霞鹜文楷 LXGW WenKai 为例），覆盖字体配置链路解析、CDN 引入（`Head.tsx` 注入 + `custom.scss` 变量覆盖 + `quartz.config.yaml` 配置）、本地构建验证与发布流程，以及 `@import` 打包报错、`og-image` 网络依赖、字体子集按需加载等常见坑。

### 变更

- section8 导读表、总规划与 `02_配置与本地构建` 交叉链接同步补充字体配置指引。
